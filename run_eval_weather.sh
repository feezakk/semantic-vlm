...
for EXP in "${EXPS[@]}"; do
  ...
  for SEED in "${SEEDS[@]}"; do
    ...
    TRAIN_LOGDIR="/data/feeza/logs_weather/${TASK}/${NAME}/seed_0"
    ...
    EVAL_LOGDIR="/data/feeza/logs_eval_weather/${TASK}/${NAME}/seed_${SEED}"
    mkdir -p "$EVAL_LOGDIR"
    LOG_FILE="${EVAL_LOGDIR}/eval.log"

    echo "[RUN] baseline=${NAME} seed=${SEED} port=${PORT}"
    if ! LOG_FILE="$LOG_FILE" ./eval_dm3_vlm_weather.sh "$PORT" "$GPU" "$CKPT" \
        --task "$EVAL_TASK" \
        $OVERRIDES \
