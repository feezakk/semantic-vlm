...
def train(agent, env, eval_env, replay, eval_replay, logger, args):
    ...
    def _csv_log(ep, ep_info, is_eval=False):
        ...
        T = len(rewards)  # rewards from ep["reward"]

        def _safe_series(dct, key):
            """Return numpy array (possibly empty) for key from ep_info/ep."""
            if key in dct:
                return np.asarray(dct[key])
            return np.asarray([])

        # Pull per-step info series (these are the ones crashing)
        off_center   = _safe_series(ep_info, "off_center_m")
        heading_err  = _safe_series(ep_info, "heading_error")
        dist_goal    = _safe_series(ep_info, "dist_to_goal_m")
        progress_m   = _safe_series(ep_info, "progress_m")
        speed_kmh    = _safe_series(ep_info, "speed_kmh")
        throttle_cmd = _safe_series(ep_info, "throttle_cmd")
        steer_cmd    = _safe_series(ep_info, "steer_cmd")

        # If info is missing for the reset/first transition, ep_info arrays are often T-1.
        # Compute how many leading steps are missing in ep_info relative to rewards.
        offset = T - len(off_center) if len(off_center) > 0 else 0
        # Clamp offset to a sane range
        offset = int(np.clip(offset, 0, T))

        def _at(arr, t, default=np.nan):
            """Index arr using t-offset. Returns default if out of range."""
            if arr is None:
                return default
            if len(arr) == 0:
                return default
    ...
    while step < args.steps:
        if should_eval(step):
            ...
            if improved:
                ...
                # Save stable "latest best"
                best_ckpt.save()

                # ALSO save a step-stamped snapshot so the step is visible from the filename
                best_step_path = logdir / f"best_step_{int(step):09d}.ckpt"
                best_step_ckpt = embodied.Checkpoint(best_step_path)
                best_step_ckpt.step = step
                best_step_ckpt.agent = agent
                # (Optional) include replay too, but it can be huge:
                # best_step_ckpt.replay = replay
                best_step_ckpt.save()
